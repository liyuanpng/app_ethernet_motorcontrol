...
static void _miiCommitBuffer(struct miiData &this, unsigned int currentBuffer, unsigned int length, chanend notificationChannel) {
    ...
    _set(this.wrPtr[bn]-4, length);       // record length of current packet.
    ...
    if (this.wrPtr[bn] > this.lastSafePtr[bn]) {  // This may be too far.
        if (this.freePtr[bn] != this.firstPtr[bn]) {// Test if head of buf is free
            _set(this.wrPtr[bn]-4, 0);          // If so, record unused tail.
            this.wrPtr[bn] = this.firstPtr[bn] + 4; // and wrap to head, and record that
            _set(this.wrPtr[bn]-4, 1);          // this is now the head of the queue.
            ...
            _set(this.wrPtr[bn]-4, 1);          // this is still the head of the queue.
        }
    } else {                                       // room in tail.
        _set(this.wrPtr[bn]-4, 1);            // record that this is now the head of the queue.
...
int _mii_out_packet_(chanend c_out, int a, int length) {
    ...
    roundedLength = length >> 2;
    assign(a, roundedLength+1, _tailValues[oddBytes]);
...
void _mii_close(chanend cNotifications, chanend cIn, chanend cOut) {
    ...
    _drain(cNotifications); // disconnect channel to ourselves
	outct(cOut, 1);        // disconnect channel to output - stops mii
    chkct(cOut, 1);
    _drain(cIn);            // disconnect input side.
}
